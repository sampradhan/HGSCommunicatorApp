...
class NewMessage extends React.Component<INewMessageProps, formState> {
        ...
        if (response.data) {
            ...
            if (this.state.templateType === this.localize("PDFUpload")) {
                ...
                if (file.type.includes('image')) {
                    ...
                    setCardEmailTemplate(this.card, response.data, this.state.title, this.state.author, EmailLink, this.state.summary)
                    this.updateCard();
                }
                else {
                    this.setState({
    ...
    public setDefaultCard = (card: any) => {
        ...
        if (this.state.templateType === this.localize("ImageUpload")) {
            ...
            setCardEmailTemplate(card, imgUrl, titleAsString, authorAsString, email, summaryAsString)
        }
            ...
            this.setState({
                ...
                if (this.state.templateType === this.localize("ImageUpload")) {
                    ...
                    if (draftMessageDetail.imageLink !== "") {
                        ...
                        const EmailLink = "[" + draftMessageDetail.emailTitle + "](" + draftMessageDetail.imageLink + ")"
                        setCardEmailTemplate(this.card, draftMessageDetail.emailBody, draftMessageDetail.title, draftMessageDetail.author, EmailLink, draftMessageDetail.summary)
                    }
                }
                                            ...
                                            <Flex.Item size="size.half">
                                                <Flex column className="formContentContainer">
                                                    ...
                                                    <Flex gap="gap.smaller" vAlign="end" className="inputField" styles={{ marginTop: "10px" }}>
                                                        ...
                                                        <input type="file" id="upload" style={{ display: 'none' }} onChange={value => this.onFileChoose(value)} accept=".png, .jpg, .jpeg, .gif" />
                                                        <Flex.Item push>
    ...
    private postDraftMessage = async (draftMessage: IDraftMessage) => {
        try {
            console.log("create draft messsage", draftMessage.AdaptiveCardContent);
